...
## Table of Contents
- [Basic Chat Schema](#basic-chat-schema)
- [Thread Management](#thread-management)
- [AI Integration with Actions](#ai-integration-with-actions)
...
- **Long conversations** - Implement context windowing or summarisation
...
The official agent component provides higher-level abstractions for building AI agents.
...
const threadId = await agent.createThread(ctx, {
  userId: args.userId,
  metadata: { title: "New Conversation", createdAt: Date.now() },
});

const threads = await agent.listThreads(ctx, { userId: args.userId });
...
const embedding = await agent.embed(ctx, content);
...
- AI Docs: https://docs.convex.dev/ai
